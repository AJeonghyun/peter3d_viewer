...
7. 생성된 1536×1152 결과 시트(4×3, 셀당 384×384)를 열어 얼굴, 색,
   옷 무늬, 오른쪽 옆모습, 팔다리 교차 동작을 검수합니다.
8. 전체 시트와 대기·좌우 걷기·손 흔들기 재생 미리보기에 문제가 없으면
   `검수 완료 · 페이지 3에 적용`을 누릅니다. 승인 전 결과는 페이지 3에
   표시되지 않으며 문제가 있으면 `12컷 다시 생성`으로 교체할 수 있습니다.

브라우저는 네 기준점으로 기울기와 원근을 자동 보정한 뒤 공통 크기로 정렬하고,
고정 실루엣으로 캐릭터를 잘라 OpenAI 이미지 편집 API에 전달합니다. AI에는
닉네임창이 아닌 학생이 그린 전체 캐릭터를 전달하며 얼굴·머리·체형·상하의·무늬·
신발·손그림 질감을 유지한 채 대기 4컷, 오른쪽 90도 옆모습 걷기 4컷,
손 흔들기 4컷을 한 장에 생성합니다. 왼쪽 이동은 웹에서 같은 걷기 프레임을
수평 반전해 표현합니다. 생성 버튼을 누를 때마다 유료 API 사용량이 발생하므로
결과가 필요한 조만 한 번씩 처리하고 실패한 조만 재시도하세요.
입력 그림 보존 강도는 기본적으로 `OPENAI_IMAGE_INPUT_FIDELITY=high`이며,
필요할 때만 배포 환경 변수로 변경합니다.
