...
1. 갈릴리 월드 상단의 `운영실` 버튼으로 관리 화면 진입
2. 조 이름, 대표색, 상징, 설명 입력
3. 색칠 그림으로 공용 GLB를 생성하거나 기존 애니메이션 GLB를 직접 등록
4. 새로 생성하는 경우 변환 작업 목록에서 3D·리깅·대기·걷기 완료 여부 확인
5. 모델 보관함에서 적용할 조들을 선택하고 한 번에 배정
6. 실패 모델은 팔다리가 잘 보이게 다시 촬영해 재등록
7. 프로그램 결과에 따라 달란트와 성품 변화 입력
8. iPad 월드에서 조별 현황 확인
9. 발표 전 네트워크를 끊고 필요한 화면이 동작하는지 리허설
